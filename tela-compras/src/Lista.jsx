          ...
          {pendentes.map(item => (
            <li key={item.id} className="item-card pendente" data-testid={`item-pendente-${item.id}`}>
              <div className="item-info">
                ...
                {item.idoso && <span style={{ marginLeft: '10px', color: '#6b7280', fontSize: '0.85rem', backgroundColor: '#f3f4f6', padding: '2px 8px', borderRadius: '4px', fontWeight: 'bold' }}>👴 {item.idoso}</span>}
                <br />
